...
//Make PRINT 1 to display generated random array elements and its sorted version
...
void insertionSort(int *A, int start, int end){
	...
	if(end > start) {
		...
		for(i = start + 1; i <= end; i++){
			value = A[i];
			hole = i;
			while(hole > start && A[hole - 1] > value){
				A[hole] = A[hole - 1];
				hole = hole - 1;
			}
			A[hole] = value;
...
void modifiedQuickSort(int *A, int start, int end) {
		
	if(start < end) {
		
		if((end - start) < 12) { //If subarray's size is less than 12 then sort with insertionSort
			insertionSort(A, start, end); 
			
		} else { //If subarray's size is greater then 12 sort with quickSort
			//printf("\n*****quicksort******\n");
			int s = partition(A, start, end); //split position
			modifiedQuickSort(A, start, s - 1); //sort before the split
			modifiedQuickSort(A, s + 1, end); //sort after the split
		}
	}
}

int main() {
	//To randomize numbers by using current time
	srand(time(0));
	//For CPU running time measurement
	clock_t start, end;
	double cpu_time_used;
	
	printf("MODIFIED QUICKSORT ALGORITHM\n*****************************\n\n");
	...
	if(type == 1) {
		...
		#if PRINT == 1
		printf("After modified quick sort\n");
